...
The **"Deal Team & Advisors"** task drives the Key Contacts section. Under `[Lyric]` and `[Advisors]` headers, one contact per line as `Name | Role | Detail (optional)`. Lines without a pipe are ignored.
