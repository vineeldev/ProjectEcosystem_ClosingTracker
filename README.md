...
The site is password protected at the application level. Set a `SITE_PASSWORD` environment variable in Vercel (Settings > Environment Variables) alongside `ASANA_PAT`, then redeploy. Visitors land on `index.html` (the Lyric x SalxCo landing page), enter the shared password, and get a signed 7-day session cookie. `middleware.js` gates `/tracker.html`, `/api/tracker`, and `/api/pdf`; without a valid cookie, pages redirect to the landing and APIs return 401. Changing `SITE_PASSWORD` invalidates all existing sessions.

Artist cards on the landing page read images from `/assets/` (the-weeknd.jpg, playboi-carti.jpg, future.jpg, lil-baby.jpg, metro-boomin.jpg). Use only properly licensed or approved imagery; until files exist the cards show designed monogram placeholders.

## Files

- `index.html` - landing page with the password gate.
- `tracker.html` - the tracker page. The 90-day frame (start date, phases, milestones) is static config at the top of the script block. Everything else loads live.
